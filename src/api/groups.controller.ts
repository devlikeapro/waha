...
import {CreateGroupRequest, DescriptionRequest, ParticipantsRequest, SubjectRequest} from "../structures/groups.dto";
...
export class GroupsController {
    ...
    @SessionApiParam
    @ApiOperation({summary: "Invalidates the current group invite code and generates a new one."})
    revokeInviteCode(
        @SessionParam session: WhatsappSession,
        @Param('id') id: string,
    ): Promise<string> {
    ...
    @SessionApiParam
    @ApiOperation({summary: 'Removes a list of participants by ID to the group.'})
    removeParticipants(
        @SessionParam session: WhatsappSession,
        @Param('id') id: string,
        @Body() request: ParticipantsRequest
